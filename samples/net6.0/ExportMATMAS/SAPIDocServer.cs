...
/// <summary>
/// Pure functional IDOC server implementation
/// </summary>
    ...
    HasMaterialManager<RT>, IHasEnvRuntimeSettings
{
    public static Aff<RT, Unit> processInboundIDoc(CalledFunction<RT> cf) => cf
        ...
        .Process(data =>
            ...
            from guardTaEmpty in guardnot(string.IsNullOrWhiteSpace(state.TransactionId),
            ...
            from clientCall in cf.UseRfcContext(context =>
                ...
                from ta in tm.GetTransaction(state.TransactionId)
                    .ToEff(RfcError.Error( "failed to read transaction data", RfcRc.RFC_EXTERNAL_FAILURE))
